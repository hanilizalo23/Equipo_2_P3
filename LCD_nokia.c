...
static const uint8_t Numbers_char[10] =
{
		'0',
		'1',
		'2',
...
const spi_config_t g_spi_config = {
							...
							SPI_BAUD_RATE_8,
							SPI_FSIZE_8,
							{GPIO_D, bit_11, bit_1, bit_2, bit_14}};
...
void LCD_nokia_send_char(uint8_t character) {
  ...
  for (index = NOTHING ; index < LIMIT_1 ; index++)
...
void LCD_nokia_delay(void)
{
	...
	for(counter=0; counter < DEL_VAL; counter++)
	{
...
{
	uint8_t num2char = Numbers_char[number];
	return(num2char);
}

void LCD_nokia_send_int(uint32_t initial_factor, uint32_t value, uint8_t number_of_digits)
{
	uint32_t factor = initial_factor;
	uint32_t actual_value = value;
	uint8_t actual_digit;
	uint8_t char_digit;
	uint8_t digit_value = NOTHING;
	for(actual_digit = NOTHING; actual_digit < number_of_digits; actual_digit++)
	{
		digit_value = ((actual_value)/ factor);
		actual_value -= (digit_value * factor);
		factor = factor / DIV;
		char_digit = LCD_nokia_num2char(digit_value);
		LCD_nokia_send_char(char_digit);
	}
}
...
void LCD_nokia_send_float(float initial_factor, float value, uint8_t number_of_digits_int, uint8_t number_of_digits_decimal)
{
	...
	uint8_t char_digit;
	uint8_t digit_value = NOTHING;
	for(actual_digit = NOTHING; actual_digit < number_of_digits_int; actual_digit++)
	{
		digit_value = ((actual_value)/ factor);
		actual_value -= (digit_value * factor);
		factor = factor / DIV;
	...
	for(actual_digit = NOTHING; actual_digit < number_of_digits_decimal; actual_digit++)
	{
		digit_value = ((actual_value)/ factor);
		actual_value -= (digit_value * factor);
		factor = factor / DIV;
